\begin{titlepage}
%\frontmatter
\begin{center}

\vspace*{-3.25cm}
 \begin{minipage}[c]{0.1\textwidth}
%\hspace{-3 cm}
\includegraphics[width=2cm,height=3cm]{Figures/isimm.png}
\end{minipage}%\hspace{+1cm}
\hfill
\begin{minipage}[c]{0.75\textwidth}
\vspace*{1cm}
\begin{small}
\begin{center}
Republic of YFZ \\ Ministry of High Education and Scientific Research \\ *** \\ University of Monastir\\~~
High Institute of Computer Science and Mathematics of Monastir\\  Department of Technology \\ *** \\ 
%\multicolumn{3}{c}{\small ~ }
\end{center} 
\end{small}
\end{minipage}
\hfill
\begin{minipage}[c]{0.1\textwidth}
%\hspace*{+0.75cm}
\includegraphics[width=3cm,height=3cm]{Figures/univ.jpg}
\end{minipage}
%\vspace*{+1cm}

\begin{tabular}{lcr}
\multicolumn{3}{c}{ \Huge \textbf{Graduation Project}} \\[0.8 cm]
%\multicolumn{3}{c}{\small ~ }\\

\multicolumn{3}{c}{ \emph{ In order to obtain the}}\\
\multicolumn{3}{c}{\textbf{\bfseries National Diploma of Engineering in Applied Sciences and Technology }}\\[0.1cm]

\multicolumn{3}{c}{\emph{   }}  \\


\multicolumn{3}{c}{ \emph{ Major: }}\\
\multicolumn{3}{c}{{\bfseries  Electronics: Microelectronics}}\\[0.1cm]

\multicolumn{3}{c}{\emph{   }}  \\

\multicolumn{3}{c}{\emph{ Presented by:}}\\
\multicolumn{3}{c}{\emph{   }}  \\
\multicolumn{3}{c}{\textbf{ {\huge Najd Elaoud}}}\\
\end{tabular}
\vspace*{0.05cm}

\begin{tabular}{lcr}
	\multicolumn{3}{c}{\rule{\linewidth}{4pt}}\\
	\multicolumn{3}{c}{\huge  ~}\\
	\multicolumn{3}{c}{\textbf{\textit{Design and development of the IOBOX-BWS industrial platform
 }}} \\
 	\multicolumn{3}{c}{\textbf{\textit{  for data acquisition, processing, and control}}}\\
	\multicolumn{3}{c}{\huge  ~} \\
	\multicolumn{3}{c}{\rule{\linewidth}{4pt}}
\end{tabular}

%\vspace*{-0.1cm}

\textbf{Presented $X$ July $2026$ in front of the jury members :}
\vspace*{1cm}
\begin{normalsize}
	\begin{description}
	\item [Mrs. UNKOWN \hspace{0.75cm}  ]\hfill  President 
	\item [Mrs. UNKOWN   \hspace{0.75cm}   ] \hfill Lecturer 
	\item [Mr. AbdAssalem BenAbdali   \hspace{0.75cm} ] \hfill  Academic Supervisor 
	\item [Mr. Jamel Hmidi   \hspace{0.75cm}] \hfill  Industrial Supervisor 
	\end{description}
\end{normalsize}

\vfill
\hrulefill \small{\textbf{\textit{\hspace*{0.3cm} Academic year: 2025/2026\hspace*{0.3cm}}}} \hrulefill
\end{center}


\end{titlepage}




